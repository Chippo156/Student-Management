...
export const exportSectionsExcel = async (sections, filters = '') => {
  try {
    ...
    row5.getCell(1).value = 'DANH SÁCH LỚP HỌC PHẦN';
    ...
    row8.getCell(1).value = { richText: [{ font: { bold: true }, text: 'Tổng số lớp học phần:' }] };
    row8.getCell(2).value = sections.length;
    ...
    const headers = ['STT', 'Mã HP', 'Tên môn học', 'Lớp', 'Giảng viên', 'Học kỳ', 'Sức chứa', 'Đã đăng ký', 'Còn trống', 'Trạng thái'];
    ...
    sections.forEach((section, index) => {
      ...
      rowData.forEach((data, colIndex) => {
        ...
        cell.value = data;
        cell.alignment = {
          horizontal: (colIndex === 0 || (colIndex >= 6 && colIndex <= 8)) ? 'center' : 'left',
          vertical: 'middle'
        };
        cell.border = {
          top: { style: 'thin' }, left: { style: 'thin' },
    ...
    const fileName = `Danh_sach_lop_hoc_phan_${new Date().getTime()}.xlsx`;
    ...
    console.error('Error exporting sections Excel:', error);
