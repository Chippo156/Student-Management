...
export const exportCoursesExcel = async (courses, filters = '') => {
  try {
    ...
    row5.getCell(1).value = 'DANH SÁCH MÔN HỌC';
    ...
    row8.getCell(1).value = { richText: [{ font: { bold: true }, text: 'Tổng số môn học:' }] };
    row8.getCell(2).value = courses.length;
    ...
    const headers = ['STT', 'Mã môn học', 'Tên môn học', 'TC Lý thuyết', 'TC Thực hành', 'Tổng TC', 'Khoa', 'Mô tả'];
    ...
    courses.forEach((course, index) => {
      ...
      const rowData = [
        ...
        course.creditsLab || 0,
        (course.creditsTheory || 0) + (course.creditsLab || 0),
      ...
      rowData.forEach((data, colIndex) => {
        ...
        cell.value = data;
        cell.alignment = {
          horizontal: (colIndex === 0 || (colIndex >= 3 && colIndex <= 5)) ? 'center' : 'left',
          vertical: 'middle'
        };
        cell.border = {
          top: { style: 'thin' }, left: { style: 'thin' },
    ...
    const fileName = `Danh_sach_mon_hoc_${new Date().getTime()}.xlsx`;
