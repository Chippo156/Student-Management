...
export const exportClassesExcel = async (classes, filters = '') => {
  try {
    ...
    row5.getCell(1).value = 'DANH SÁCH LỚP HỌC';
    ...
    row8.getCell(1).value = { richText: [{ font: { bold: true }, text: 'Tổng số lớp:' }] };
    row8.getCell(2).value = classes.length;
    ...
    const headers = ['STT', 'Mã lớp', 'Tên lớp', 'Khoa', 'Chương trình', 'GVCN', 'Sĩ số', 'Khóa'];
    ...
    classes.forEach((classItem, index) => {
      ...
      const rowData = [
        ...
        classItem.classCode || '',
        classItem.className || '',
        classItem.program?.department?.departmentName || '',
        classItem.program?.programName || '',
        classItem.advisor?.user?.fullName || '',
      ...
      rowData.forEach((data, colIndex) => {
        ...
        cell.value = data;
        cell.alignment = { horizontal: colIndex === 0 || colIndex === 6 ? 'center' : 'left', vertical: 'middle' };
        cell.border = {
          top: { style: 'thin' }, left: { style: 'thin' },
          bottom: { style: 'thin' }, right: { style: 'thin' }
        };
    ...
    const fileName = `Danh_sach_lop_hoc_${new Date().getTime()}.xlsx`;
    ...
    console.error('Error exporting classes Excel:', error);
