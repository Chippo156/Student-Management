  ...
  const handleOk = async () => {
    try {
      ...
      const newAccount: BankAccount = {
        ...
        isDefault: editingAccount ? editingAccount.isDefault : bankAccounts.length === 0,
        status: 'active',
        createdDate: editingAccount ? editingAccount.createdDate : new Date().toISOString().split('T')[0],
      };
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Descriptions column={1} size="small">
                <Descriptions.Item label="Ngân hàng">{defaultAccount.bankName}</Descriptions.Item>
                <Descriptions.Item label="Mã ngân hàng">{defaultAccount.bankCode}</Descriptions.Item>
                <Descriptions.Item label="Số tài khoản">
                  <Text copyable={{ text: defaultAccount.accountNumber }}>
                    {defaultAccount.accountNumber}
                  </Text>
                </Descriptions.Item>
                <Descriptions.Item label="Chủ tài khoản">{defaultAccount.accountHolder}</Descriptions.Item>
              </Descriptions>
            </Col>
            <Col span={12}>
              <Descriptions column={1} size="small">
                <Descriptions.Item label="Chi nhánh">{defaultAccount.branchName}</Descriptions.Item>
                ...
                <Descriptions.Item label="Ngày tạo">{defaultAccount.createdDate}</Descriptions.Item>
              </Descriptions>
            </Col>
          </Row>
        </Card>
                ...
                <List.Item.Meta
                  ...
                  title={
                    <Space>
                      ...
                      <Tag color="green">{getAccountTypeText(account.accountType)}</Tag>
                    </Space>
                  }
                  description={
                    <Space direction="vertical" style={{ width: '100%' }}>
                      <Text><strong>Ngân hàng:</strong> {account.bankName}</Text>
                      <Text>
                        <strong>Số tài khoản:</strong> 
                        <Text copyable={{ text: account.accountNumber }} style={{ marginLeft: 8 }}>
                          {account.accountNumber}
                        </Text>
                      </Text>
                      <Text><strong>Chủ tài khoản:</strong> {account.accountHolder}</Text>
                      <Text><strong>Chi nhánh:</strong> {account.branchName}</Text>
                      <Space>
                        <Text><strong>Trạng thái:</strong></Text>
                        <Tag color={account.status === 'active' ? 'green' : 'red'} icon={<CheckCircleOutlined />}>
                          {account.status === 'active' ? 'Hoạt động' : 'Không hoạt động'}
                        </Tag>
                        <Text type="secondary">Tạo ngày: {account.createdDate}</Text>
                      </Space>
                    </Space>
                  }
                />
        ...
        <Form form={form} layout="vertical">
          ...
          <Form.Item
            ...
            rules={[
              ...
              { pattern: /^\d{10,16}$/, message: 'Số tài khoản phải có 10-16 chữ số!' }
            ]}
          >
