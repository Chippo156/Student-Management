...
const BHYTPage: React.FC = () => {
  ...
  const [medicalHistory, setMedicalHistory] = useState<MedicalRecord[]>(insuranceInfo.medicalHistory);
  ...
  const totalCost = medicalHistory.reduce((sum, record) => sum + record.cost, 0);
  const totalCovered = medicalHistory.reduce((sum, record) => sum + record.covered, 0);
  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      <Card 
        ...
        extra={
          <Tag color={getStatusColor(insuranceInfo.status)} icon={<CheckCircleOutlined />}>
        ...
        <Row gutter={[16, 16]}>
          <Col span={12}>
            <Descriptions column={1} size="small">
              <Descriptions.Item label="Số thẻ">{insuranceInfo.cardNumber}</Descriptions.Item>
              <Descriptions.Item label="Họ và tên">{insuranceInfo.holderName}</Descriptions.Item>
              <Descriptions.Item label="Ngày sinh">{insuranceInfo.dateOfBirth}</Descriptions.Item>
          ...
          <Col span={12}>
            <Descriptions column={1} size="small">
              <Descriptions.Item label="Ngày cấp">{insuranceInfo.issueDate}</Descriptions.Item>
              <Descriptions.Item label="Ngày hết hạn">{insuranceInfo.expiryDate}</Descriptions.Item>
              <Descriptions.Item label="Nơi cấp">{insuranceInfo.issuedBy}</Descriptions.Item>
              <Descriptions.Item label="Nơi ĐK KCB">{insuranceInfo.hospitalRegistered}</Descriptions.Item>
            </Descriptions>
          </Col>
        </Row>
      ...
      >
        {medicalHistory.length > 0 ? (
          <List
            itemLayout="vertical"
            dataSource={medicalHistory.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())}
            renderItem={(record) => (
              <List.Item
                actions={[
                ...
                <List.Item.Meta
                  ...
                  description={
                    <Space direction="vertical" style={{ width: '100%' }}>
                      ...
                      <Space>
                        <Text>
                          <strong>Chi phí:</strong> {record.cost.toLocaleString()}₫
                        </Text>
                        <Text type="success">
                          <strong>BHYT chi trả:</strong> {record.covered.toLocaleString()}₫
                        </Text>
                        <Text type="secondary">
                          <strong>Tự trả:</strong> {(record.cost - record.covered).toLocaleString()}₫
                        </Text>
                      </Space>
                      {record.notes && (
                        <Text type="secondary">
          ...
          <Empty description="Chưa có hồ sơ khám chữa bệnh nào" />
        )}
      </Card>

      {/* Modal */}
      <Modal
        title={editingRecord ? 'Chỉnh sửa hồ sơ khám bệnh' : 'Thêm hồ sơ khám bệnh'}
        open={isModalVisible}
        onOk={handleOk}
        ...
        <Form form={form} layout="vertical">
          ...
          <div style={{ display: 'flex', gap: 16 }}>
            ...
            <Form.Item
              name="covered"
              label="BHYT chi trả (₫)"
              rules={[{ required: true, message: 'Vui lòng nhập số tiền BHYT chi trả!' }]}
              style={{ flex: 1 }}
            >
              <Input type="number" placeholder="0" />
            </Form.Item>
