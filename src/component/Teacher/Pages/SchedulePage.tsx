...
const SchedulePage: React.FC = () => {
  ...
  const getStatusChip = (status: string) => {
    const statusConfig = {
      upcoming: { label: 'Sắp tới', color: 'warning' as const },
  ...
  // Nhóm lịch theo ngày
  const groupScheduleByDate = (scheduleData: any[]) => {
    const grouped = scheduleData.reduce((acc: any, item: any) => {
  ...
  const todaySchedule = schedule.filter((item: any) => 
    ...
    <Box sx={{ flexGrow: 1, p: 3 }}>
          ...
          <Card sx={{ backgroundColor: '#e8f5e8', height: '100%' }}>
            <CardContent>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={6}>
                  <Typography variant="h4" component="div" sx={{ fontWeight: 'bold' }}>
                    {new Set(schedule.map((item: any) => item.courseId)).size}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    Môn học
                  </Typography>
      ...
      <Paper sx={{ mt: 3 }}>
        ...
        {Object.keys(groupedSchedule).length > 0 ? (
          Object.entries(groupedSchedule).map(([date, daySchedule]: [string, any]) => (
            <Box key={date} sx={{ mb: 3 }}>
              <Box sx={{ p: 2, backgroundColor: '#f5f5f5', borderBottom: '1px solid #e0e0e0' }}>
                <Typography variant="subtitle1" sx={{ fontWeight: 'bold' }}>
                  {getDayOfWeek(date)} - {new Date(date).toLocaleDateString('vi-VN')}
                </Typography>
              </Box>
                      ...
                      <TableRow key={index} hover>
                        ...
                        <TableCell>
                          <Button 
                            variant="outlined" 
                            size="small"
                            onClick={() => {/* Navigate to class detail */}}
                          >
                            Chi tiết
                          </Button>
                        </TableCell>
