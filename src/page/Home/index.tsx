...
function StudentManagementLayout(): JSX.Element {
  const [openMenus, setOpenMenus] = useState<OpenMenusState>({});
  const [selectedKey, setSelectedKey] = useState<string>("home");
  const [collapsed, setCollapsed] = useState<boolean>(false);

  const handleMenuClick = (key: string, hasChildren: boolean): void => {
    if (hasChildren) {
  ...
  // Content mapping for each menu key
  const contentMap: Record<string, React.ReactNode> = {
    ...
    curriculum: <div style={{ padding: '24px' }}>
      <Typography variant="h4" gutterBottom>Chương trình khung</Typography>
      <Typography>Component chương trình khung đang được phát triển...</Typography>
    </div>,
    "register-course": <div style={{ padding: '24px' }}>
      <Typography variant="h4" gutterBottom>Đăng ký học phần</Typography>
      <Typography>Component đăng ký học phần đang được phát triển...</Typography>
    </div>,
    debt: <div style={{ padding: '24px' }}>
      <Typography variant="h4" gutterBottom>Tra cứu công nợ</Typography>
      <Typography>Component tra cứu công nợ đang được phát triển...</Typography>
    </div>,
    pay: <div style={{ padding: '24px' }}>
      <Typography variant="h4" gutterBottom>Thanh toán trực tuyến</Typography>
