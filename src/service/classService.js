...
export const classService = {
  ...
  getAdviserClasses: async () => {
    try {
      const response = await axios.get('/api/Class/dropdown/adviser');
      if (response?.success === false) {
        const errData = response?.data;
        if (Array.isArray(errData) && errData.length > 0) {
          message.error(errData[0]);
          ...
          message.error(
            response?.message || 'Lấy danh sách lớp chủ nhiệm thất bại'
          );
        }
        return null;
      }
      ...
      if (error.response && error.response.data) {
        ...
        if (Array.isArray(errData) && errData.length > 0) {
          ...
          message.error(
            error.response.data.message || 'Lấy danh sách lớp chủ nhiệm thất bại'
          );
        }
      } else {
        message.error(error.message || 'Lấy danh sách lớp chủ nhiệm thất bại');
      }
      return null;
    }
  },
